    ...
    def _validate(cls, value: str) -> None:
        if len(value) < 2:
            raise ValidationError(
                f"Name must be at least 2 characters long: {len(value)}"
            )
        if len(value) > 50:
            raise ValidationError(
                f"Name must be at most 50 characters long: {len(value)}"
            )
        if not value.isalpha():
            raise ValidationError(
                f"Name must contain only alphabetic characters: {value}"
            )
