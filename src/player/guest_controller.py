"""
This module contains the guest controller. It defines the routes for managing guests.
"""
...
def get_guests(service: GuestService):
    ...
    return [GuestDto.load(guest).model_dump() for guest in service.get_guests(host_id)]
...
def get_guest_achievements(service: GuestService, guest_id: UUID):
    ...
    return [
        AchievementDto.load(achievement).model_dump() for achievement in achievements
    ]
